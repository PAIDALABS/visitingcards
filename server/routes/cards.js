...
router.put('/:id', async function (req, res) {
    try {
        ...
        if (existingCard.rows.length === 0) {
            // New card — enforce plan limits (count only active cards)
            var countResult = await db.query('SELECT COUNT(*) FROM cards WHERE user_id = $1 AND active = true', [req.user.uid]);
        ...
        // Block edits to inactive cards
        if (existingCard.rows.length > 0 && !existingCard.rows[0].active) {
