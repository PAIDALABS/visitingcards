...
function sendAiInsightEmail(toEmail, analysis, periodDays) {
    // Convert simple markdown to HTML (headers + bold + line breaks)
    var htmlBody = analysis
        .replace(/^### (.+)$/gm, '<h3 style="color:#a78bfa;margin:20px 0 8px;font-size:15px">$1</h3>')
        .replace(/^## (.+)$/gm, '<h2 style="color:#c4b5fd;margin:24px 0 8px;font-size:17px">$1</h2>')
        .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
        .replace(/\n/g, '<br>');
    var now = new Date();
    var dateStr = now.toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    var body =
        '<h2 style="color:#fff;margin:0 0 8px">AI Funnel Insights</h2>' +
        '<p style="color:#9ca3af;font-size:13px;margin:0 0 24px">' + dateStr + ' &nbsp;·&nbsp; Last ' + periodDays + ' days</p>' +
        '<div style="background:#111827;border-radius:8px;padding:20px;border:1px solid #374151;font-size:14px;line-height:1.7;color:#e5e7eb">' +
        htmlBody +
        '</div>' +
        '<p style="color:#6b7280;font-size:12px;margin-top:24px">Generated by Claude. Review in the <a href="' + BASE_URL + '/super-admin#funnel" style="color:#a78bfa">Admin Funnel Dashboard</a>.</p>';
    return sendEmail(toEmail, 'CardFlow Funnel Insights — ' + dateStr, wrapHtml('AI Funnel Insights', body));
}
...
module.exports = {
    ...
    sendAiInsightEmail: sendAiInsightEmail,
